...
- Конкурентность ограничена `Semaphore(15)` — не более 15 одновременных запросов
- При ошибке `429 Too Many Requests` парсер делает паузу и повторяет запрос
- Для устойчивости к `ConnectionTimeoutError` используется `tenacity` (5 попыток с интервалом 2 сек)
- Данные пишутся в `res.json` построчно (NDJSON), что позволяет восстановить частичный результат при падении

---

## Что можно улучшить в production-версии

Решение реализовано в рамках тестового задания. В боевом варианте имеет смысл рассмотреть:

- **Хранилище**: заменить NDJSON на SQLite/PostgreSQL через `aiosqlite` — дедупликация, checkpoint (возобновление с места падения), нормальные запросы вместо pandas-маски
- **Конфигурация**: вынести все константы в `.env` + `pydantic-settings`
- **Структура**: разбить на модули — `models.py`, `scraper.py`, `storage.py`, `config.py`
- **Прокси**: ротация `N прокси × сессия × семафор` с автоматическим cooldown при бане
- **Логирование**: заменить `print` на `logging` с уровнями и контекстом
- **Тесты**: unit-тесты на Pydantic-валидаторы и basket-маппер
